...
DrawableObject* Factory::createAssimpModel(std::string filePath, const char* textureName, glm::mat4 transformationMatrix)
{
	AssimpModel* assimpModel = new AssimpModel();
	assimpModel->load(filePath);
	assimpModel->loadTexture(textureName);

	return new DrawableObject( (Model*) assimpModel, true, new ShaderProgram("assimpShader.frag", "assimpShader.vert"), new Transformation(transformationMatrix));
...
void Factory::addTreeOnPosition(Scene* scene, glm::vec3 pos)
{
	...
	scene->addDrawableObject(this->createAssimpModel("AssimpModels/tree/tree.obj", "AssimpModels/tree/tree.png", M0), TEXTURE);
}
...
Scene* Factory::createSkyBoxScene()
{
	...
	scene->addDrawableObject(this->createObjectTexture(glm::scale(glm::mat4(1.0f), glm::vec3(40.0f)), "grass.png"), TEXTURE);
...
Scene* Factory::createAssimpScene()
{
	Scene* scene = new Scene();
	scene->setSkybox(this->createSkyBox());


	scene->addDrawableObject(this->createObjectTexture(glm::scale(glm::mat4(1.0f), glm::vec3(100.0f,0.0f,100.0f)), "grass.png"), TEXTURE);

	scene->addDrawableObject(this->createAssimpModel("AssimpModels/zombie/zombie.obj","AssimpModels/zombie/zombie.png", glm::translate(glm::mat4(1.0f), glm::vec3(-5.0f, 0.0f, 0.0f))), TEXTURE);
	scene->addDrawableObject(this->createAssimpModel("AssimpModels/house/model.obj", "AssimpModels/house/model.png", glm::translate(glm::mat4(1.0f), glm::vec3(10.0f, 0.0f, -1.0f))), TEXTURE);
