...
void ShaderProgram::bindAssimpTexture(GLuint textureUnitId, GLuint textureID)
{
	glActiveTexture(textureUnitId);
	glBindTexture(GL_TEXTURE_2D, textureID);

	GLint texture = glGetUniformLocation(this->Program, "textureUnitID");
	...
	glUniform1f(texture, textureUnitId);
}
